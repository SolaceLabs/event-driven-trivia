...
class ConsoleClient {
  constructor(name = 'console') {
    console.log(this.getTime(), 'Create ConsoleClient');
  ...
  init = function () {
    ...
    try {
      this.session = this.solace.SolclientFactory.createSession({
        ...
        userName: username,
        password,
        clientName: 'trivia/' + this.name + '-' + shortHash('-' + (new Date()).getMilliseconds() + '+')
      });
    } catch (error) {
    ...
    this.session.on(this.solace.SessionEventCode.MESSAGE, (message) => {
      // console.log(this.getTime(), `${this.name}: Message received`);
  ...
  publish = function (topic, messageText = '') {
    if (this.session !== null) {
      ...
      this.name === 'monitor'
        ? console.log(this.getTime(), `${this.name}: Publishing message to topic ${topic} with ${JSON.stringify(messageText)}...`)
        : console.log(this.getTime(), `${this.name}: Publishing message to topic ${topic} ...`);
      try {
