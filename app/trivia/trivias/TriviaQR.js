...
export default function TriviaQR(props) {
  const { data, open, close } = props;
  const classes = useStyles();

  const [qrCode, setQRCode] = React.useState(data[14]);
  const [gameUrl, setGameUrl] = React.useState(false);

  useEffect(async () => {
    let location = window.location.href;
    location = location.substring(0, location.indexOf('/app/trivia')) + '/game.html?code=' + data[13];
    setGameUrl(location);

    const response = await api.getTriviaQRCode({ url: location });
    if (!response.success) {
